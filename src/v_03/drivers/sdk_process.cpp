...
bool sdk_process::del_0(void)	{
    ...
    if(g_shmFd == -1) {
		...
		while(g_pid != g_msg_ptr->gActivePid)	{}
		...
		if(0 == g_msg_ptr->gDel) {
			...
			if(l_last_process) {
				...
				for(uint8_t i=0;i<PROCESS_MAX_NUMBER;i++) {
					std::cout << "Process id:" << (int) i << " --- pid: " << g_msg_ptr->process_buffer[i].pid << std::endl;
				}
			}
...
uint16_t sdk_process::read(void)	{
	uint16_t l_data = SDK_PROCESS_STATUS__NO_DATA;
		...
		if(
			...
			if(g_msg_ptr->gDataWriterPid)	{
				...
				if(g_pid == g_msg_ptr->gDataWriterPid)	{
					// the current process is the process that write/add data to packket buffer
					// all process have read data
				} else {
					// the current process is not the process that add data to packket buffer
					// therefore there are a data to read
					l_data = g_msg_ptr->buffer[0];
				}
			}
		}
    }
...
bool sdk_process::write(char c)	{
    ...
    if(g_shmFd != -1) {
		...
		// start the procedure to modify sahred memory data
		// N.B.: it is necessary otherwise next() don't work
		start();
...
void sdk_process::write_end(void)	{
    ...
    if(g_shmFd != -1) {
		...
		// check data buffer status
		if(g_msg_ptr->gDataWriterPid)	g_msg_ptr->gDataWriterPid = 0;
